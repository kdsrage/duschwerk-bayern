...
function WalkIn({ w, h, t, glassMat, metalMat, rahmentyp }) {
  ...
  return (
    <group>
      ...
      {/* Seitenprofile — teilgerahmt und vollgerahmt */}
      {!rahmenlos && (
        <>
          <mesh position={[-w / 2 + pw / 2, 0, 0]}>
            <boxGeometry args={[pw, h, phe]} />
      ...
      {/* Quer-Profile (oben + unten) — NUR vollgerahmt */}
      {voll && (
        <>
          <mesh position={[0, h / 2 - pw / 2, 0]}>
            <boxGeometry args={[w, pw, phe]} />
      ...
      {/* Rahmenlos: 2 runde Glashalter / Wandscharniere an der linken Seite */}
      {rahmenlos && [h / 2 - 0.12, -h / 2 + 0.12].map((y, i) => (
        <group key={i} position={[-w / 2, y, 0]}>
          {/* Äußere Klemmplatte (Raumseite) */}
          <mesh rotation={[0, Math.PI / 2, 0]} position={[0.006, 0, 0]}>
            <cylinderGeometry args={[0.026, 0.026, 0.010, 18]} />
            <primitive object={metalMat} attach="material" />
          </mesh>
          {/* Innere Klemmplatte (Duschseite) */}
          <mesh rotation={[0, Math.PI / 2, 0]} position={[-0.006, 0, 0]}>
            <cylinderGeometry args={[0.020, 0.020, 0.008, 18]} />
            <primitive object={metalMat} attach="material" />
          </mesh>
          {/* Wandbolzen */}
...
function Drehtuer({ w, h, t, glassMat, metalMat, rahmentyp, doorOpen }) {
  ...
  return (
    <group>
      ...
      {/* Mittelprofil Türanschlag — nur wenn nicht rahmenlos */}
      {rahmentyp !== 'rahmenlos' && (
        <mesh position={[midX, 0, 0]}>
          <boxGeometry args={[P, h - pw * 2, PH * 1.5]} />
          <primitive object={metalMat} attach="material" />
        </mesh>
      )}
    </group>
  ...
  return (
    <group>
      ...
      <group ref={frontRef} position={[frontX, 0, t * 0.55]}>
        ...
        {/* Außengriff (Raumseite, +Z) */}
        <mesh position={[pW * 0.38, 0, t / 2 + 0.018]}>
          <cylinderGeometry args={[0.008, 0.008, 0.22, 10]} />
          <primitive object={metalMat} attach="material" />
        </mesh>
        {/* Innengriff (Duschseite, -Z) — versetzt auf gegenüberliegender Glasseite */}
        <mesh position={[pW * 0.38, 0, -(t / 2 + 0.018)]}>
          <cylinderGeometry args={[0.008, 0.008, 0.22, 10]} />
          <primitive object={metalMat} attach="material" />
        </mesh>
      ...
      {/* Panel hinten (rechts) — statisch, kein Griff */}
